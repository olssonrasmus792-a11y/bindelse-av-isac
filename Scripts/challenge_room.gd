...
@export var enemy_scenes = [
	preload("res://Scenes/Enemies/ghosty.tscn")
]
...
var min_player_distance := 1000.0
...
func _ready() -> void:
	...
	chest.label_2.text = "Kill All Ghosts!"
	chest.item_amount = 2
...
func get_valid_spawn_position():
	...
	if player:
		var to_pos = pos - player.global_position
		var dist = to_pos.length()

		if dist < min_player_distance and dist != 0:
			var dir = to_pos.normalized()

			# push it out to minimum distance
			pos = player.global_position + dir * min_player_distance
...
func spawn_enemies():
	...
	var enemy_count := int(20 + (20 * 0.2 * GameState.get_item_count("Sword")))
	
	for item in GameState.taken_items:
		if item.name == "Sword":
			item.tracked_stat_values[1] += int(20 * 0.2 * GameState.get_item_count("Sword"))
			break
	...
	for x in range(enemy_count):
		...
		var base_pos = get_valid_spawn_position()
